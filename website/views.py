...
db = mysql.connector.connect(
    host="localhost", user="root", password="", database="cearis_db"
)
...
def student_rep_login():
    if request.method == "POST":
        ...
        print(
            f"Attempting login with student_number: {student_number}, password: {password}"
        )

        # Query the database to check for authentication
        mycursor.execute(
            "SELECT * FROM users WHERE studentNumber = %s AND password = %s",
    ...
    return render_template(
        "student-rep/student-rep-login.html", error="Invalid student number or password"
    )
...
def student_rep_home():
    ...
    mycursor.execute(
        "SELECT COUNT(*) FROM cpe_dept_rooms WHERE status = 'under maintenance'"
    )
    maintenance_count = mycursor.fetchone()[0]

    return render_template(
        "student-rep/student-rep-home.html",
...
def student_rep_home_rooms():
    ...
    for room_number in room_numbers:
        mycursor.execute(
            "SELECT * FROM cpe_dept_rooms WHERE roomNumber = %s", (room_number[0],)
        )
        ...
        serializable_room_data = {
            ...
            "status": str(
                room_data[2]
            ),  # Convert to a serializable type (e.g., string)
    ...
    return render_template(
        "student-rep/student-rep-home-rooms.html",
...
def admin_login():
    if request.method == "POST":
        ...
        # Print the credentials for debugging purposes
        print(
            f"Attempting login with faculty_number: {faculty_number}, password: {password}"
        )

        # Query the database to check for authentication
        mycursor.execute(
            "SELECT * FROM admin WHERE facultyNumber = %s AND password = %s",
            (faculty_number, password),
        )
    ...
    return render_template(
        "admin/admin-login.html", error="Invalid faculty number or password"
    )


@views.route("/student-rep/profile")
def student_rep_home_profile():
    return render_template("student-rep/student-rep-profile.html")
